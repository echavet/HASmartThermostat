...
climate:
  - platform: smart_thermostat
    ...
    kd: 2000
    pwm: 00:15:00
    # Optional: cut heating when window is open
...
- [Practical PID Process Dynamics with Proportional Pressure Controllers](https://clippard.com/cms/wiki/practical-pid-process-dynamics-proportional-pressure-controllers)
...
- **outdoor_sensor** (Optional): entity_id for an outdoor temperature sensor, outdoor_sensor.state
  must be temperature.
- **window_sensor** (Optional): entity_id for a window/door sensor. When set and the window is open,
  the thermostat does not change its hvac mode but forces the output to 0 (cuts heating/cooling).
  When the window closes, the PID integral is reset to resume regulation cleanly.
...
- **kp** (Recommended): Set PID parameter, proportional (p) control value (float, default 100).
  _Note:_ Once the thermostat has been created, changing the configuration PID values will not
